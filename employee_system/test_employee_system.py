...
class EmployeeSystemTests(TestCase):
    ...
    def test_employee_profile_str(self):
        employee = EmployeeProfile.objects.create(
            first_name='Jordan', last_name='Hire', email='jordan@example.com', department='Finance'
        )
        self.assertEqual(str(employee), 'Jordan Hire')
    ...
    def test_employee_system_api_updates_existing_employee(self):
        EmployeeProfile.objects.create(
            first_name='Jordan', last_name='Hire', email='jordan@example.com', department='Finance', status='pending'
        )
        response = self.client.post(reverse('employee_system:create_employee_api'), data={
            'first_name': 'Jordan', 'last_name': 'Hire', 'email': 'jordan@example.com', 'department': 'Finance', 'source_application_id': 1, 'status': 'active'
        }, content_type='application/json', HTTP_AUTHORIZATION='Token class-employee-system-token')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(EmployeeProfile.objects.count(), 1)
        employee = EmployeeProfile.objects.get(email='jordan@example.com')
        self.assertEqual(employee.status, 'active')
